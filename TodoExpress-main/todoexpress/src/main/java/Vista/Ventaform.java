...
private final VentaDAO ventaActual = new VentaDAO();
    ...
    public Ventaform() {
        ...
        DefaultTableModel modeloTabla = new DefaultTableModel(); //tabla
...
String stockStr = txtcantidad.getText();       // Stock disponible
...
try {
    ...
    if (!productoExiste) {
        ...
        modelo.addRow(new Object[]{producto, precio, cantidad});//array
    }
...
public void agregarProducto(ProductoDAO producto) {
    if (ventaActual.getProductos() == null) {
        ventaActual.setProductos(new ArrayList<>()); //lista
    }
